# script to liftover cleaned BBJ from hg19 to hg38
...
trait=$1
echo "liftover ${trait} ..."
zcat ../1_cleaned_data/${trait}_bbj.tsv.gz | \
...
bcftools query -f '%CHROM\t%POS\t%REF\t%ALT[\t%ES\t%SE\t%LP]\n' | \
awk -F"\t" -v OFS="\t" '{$7=10^(-$7); print}' | \
awk 'BEGIN{print "CHR\tBP\tA2\tA1\tBETA\tSE\tPVAL"}1' |\
gzip > ${trait}_bbj.hg38.tsv.gz
